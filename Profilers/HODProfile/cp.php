...
mysqli_select_db($connect,"revised"); // Selecting Database from Server
	...
	if($Password && $repassword && $cur) 
	{
		...
		{
			$sql = mysqli_query($connect,"SELECT * FROM `revised`.`hlogin` WHERE `Username`='$Username'");
			if(mysqli_num_rows($sql) == 1)
			{
				...
				if($cur == $dbpassword)
				{
					if($query = mysqli_query($connect,"UPDATE `revised`.`hlogin` SET `Password` = '$Password' WHERE `hlogin`.`Username` = '$Username'"))
					{
